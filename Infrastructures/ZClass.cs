...
{/// <summary>
 ...
 /// <returns></returns>
    ...
    {
        //定义程序集的名称
        AssemblyName aName = new AssemblyName($"{type}OverrideAssembly");
        ...
        // 使用模块构建器创建一个类型构建器
        TypeBuilder tb = mb.DefineType(type.Name + "OverrideType", type.Attributes, type);
        
        return tb;
    }
    static Dictionary<MethodInfo, List<Attribute>> FindOverrideMethods(Type enrichType)
    {
    ...
    static void OverrideMethods(TypeBuilder tb, Dictionary<MethodInfo, List<Attribute>> keyValuePairs)
    {
        foreach (var kv in keyValuePairs)
        {
            ...
            {
                MethodBuildTool.ConstructAction(tb, IL, kv.Value, orgMethod);
            }
            else
            {
                MethodBuildTool.ConstructFunc(tb, IL, kv.Value, orgMethod);
            }
        }
    }
    public static Type Make<T>()
    {
        var typeBuilder = CreateTypeBuilder(typeof(T));
        var overrideMethods = FindOverrideMethods(typeof(T));
        OverrideMethods(typeBuilder, overrideMethods);
    ...
    public static void ConstructAction(TypeBuilder tb, ILGenerator overrideIL, List<Attribute> attris, MethodInfo orgMethod)
    {
        ...
        //定义初始化函数
        {
            ...
            for (int j = 0; j < attris.Count; j++)
            {
                ...
                //赋值
                initIL.Emit(OpCodes.Ldarg_0);
                initIL.Emit(OpCodes.Call, typeof(MethodBuildTool).GetMethod(nameof(Create)).MakeGenericMethod(attris[j].GetType()));
            ...
            for (int k = 0; k < fieldInfos.Count; k++)
            {
                ...
                initIL.Emit(OpCodes.Call, typeof(MethodBuildTool).GetMethod(nameof(Pack)));
                ////赋值到next中
